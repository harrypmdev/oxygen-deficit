...
func neighboring_coords_cached(coord: Vector2i, peering_neighbors: Array) -> Array:
	if neighbor_cache.has(coord):
		return neighbor_cache[coord]
	var neighbors = data.neighboring_coords($Map/RockTiles, coord, peering_neighbors)
...
func _widen(tm: TileMapLayer, coords: Array) -> Array:
	...
	var processed = {}
	var peering_neighbors = data.get_terrain_peering_cells(tm.tile_set, TerrainType.MATCH_TILES)
	var local_neighbor_cache = {}  # Local cache to avoid redundant calls
	
	for coord in coords:
		if !processed.has(coord):
			result[coord] = true
			# Cache neighbors for this coordinate
			if not local_neighbor_cache.has(coord):
				local_neighbor_cache[coord] = data.neighboring_coords(tm, coord, peering_neighbors)
			for neighbor in local_neighbor_cache[coord]:
...
#func create_terrain_changeset(tm: TileMapLayer, paint: Dictionary) -> Dictionary:
	...
	#var cells = paint.keys()
	#var needed_cells := []
	#var chunk_size = 100  # Process 100 cells per thread chunk
	#var total_chunks = ceil(float(cells.size()) / chunk_size)
	#
	## Delegate _widen to threads
	#var work = func(n: int):
		#var chunk = cells.slice(n * chunk_size, (n + 1) * chunk_size)
		#needed_cells.append_array(_widen(tm, chunk))
	#
	#var group_id = WorkerThreadPool.add_group_task(work, total_chunks, -1, false, "BetterTerrain")
	#paint = normalize_paint_keys(paint)
	#return {
		#"valid": true,
		#"tilemap": tm,
		#"cells": cells,
...
#HPM Version
#func apply_terrain_changeset(change: Dictionary) -> void:
	#if !change or !change.has("needed_cells"):
		#return
	#
	#var tm = change.tilemap
	#var cells = change.needed_cells
	#var paint = change.paint
	#
	#var types = {}
	#for c in cells:
		#var key = Vector2i(c.x, c.y)  # Ensure a consistent key format
		#types[c] = paint[key] if paint.has(key) else get_cell(tm, c)
	#
	#var ts_meta = _get_terrain_meta(tm.tile_set)
	#var cache = _get_cache(tm.tile_set)
	#
	## Apply the changes to the TileMapLayer
	#for coord in cells:
		#_update_tile_immediate(tm, coord, ts_meta, types, cache)

func normalize_paint_keys(dict: Dictionary) -> Dictionary:
	var normalized = {}
	for key in dict.keys():
		var coord = Vector2i(key.x, key.y)  # Convert keys to Vector2i
		normalized[coord] = dict[key]
	return normalized
